...
import {
  Character,
  CharacterStats as Stats
} from "../types/character";
import {
  EquippedItems,
  calculateGearScore,
  calculateEquipmentStats,
  getEquipmentIcon   // ✅ IMPORT SMART ICON HELPER
...
function AchievementPanel({
  ...
  const pageItems = allAchievements.slice(
    page * PER_PAGE,
    (page + 1) * PER_PAGE
  );
  ...
  return (
    ...
    >
      ...
      {pageItems.map((achievement) => {
        ...
        const color =
          rarityColors[achievement.rarity] || "#9ca3af";
...
export function CharacterStats({
  ...
  const totalPendingPoints = Object.values(
    pendingStats
  ).reduce((sum, val) => sum + val, 0);
  const remainingPoints =
    character.statPoints - totalPendingPoints;
  ...
  const confirmStats = () => {
    Object.entries(pendingStats).forEach(
      ([stat, amount]) => {
  ...
  const expProgress = Math.floor(
    (character.exp / character.expToNext) * 100
  );
  const hpPercent = Math.floor(
    (character.hp / character.maxHp) * 100
  );
  const mpPercent = Math.floor(
  ...
  const def = calcPlayerDef(
    character,
    armorBonus,
    mdefBonus
  );
  const crit = calcCritChance(
    character,
    passives.critBonus
  );
  const aspd = calcASPD(
  ...
  const previewStats = useMemo(() => {
    ...
    const {
      attack: previewAtkRange,
      passives: previewPassives
    } = calcPlayerAtk(
      previewChar,
      equipStats.weaponAtk,
      equipStats.weaponLevel,
    ...
    const previewDef = calcPlayerDef(
      previewChar,
      armorBonus,
      mdefBonus
    );
    const previewCrit = calcCritChance(
      previewChar,
      previewPassives.critBonus
    );
    const previewAspd = calcASPD(
      previewChar,
      previewPassives.aspdBonus
    ).toFixed(2);
    const previewMaxHp = calcMaxHp(
      character.level,
      previewChar.stats.vit,
      character.jobClass
    );
    const previewMaxMp = calcMaxMp(
      character.level,
      previewChar.stats.int,
      character.jobClass
    );
  ...
  const totalEquipPower = Object.values(equipped)
    ...
    .reduce(
      (sum, item) => sum + calculateGearScore(item as any),
      0
    );

  const basePower =
    character.level * 10 + character.jobLevel * 5;
  ...
  const getEquipmentBonusForStat = (
    statKey: keyof Stats
  ) => {
      ...
      >
        ...
        >
          ...
          >
            <div
              style={{
                ...
                background:
                  "linear-gradient(90deg, #b45309, #f59e0b, #b45309)",
                padding: "6px 20px",
                borderRadius: "24px",
                fontWeight: "900",
                color: "white",
          ...
          >
            ...
            >
              ...
              {(
                ["str", "agi", "vit", "int", "dex", "luk"] as const
              ).map((key) => {
                const equipBonus =
                  getEquipmentBonusForStat(key);
            ...
            >
              ...
              {renderStatWithPreview(
                "ATK",
                atkDisplay,
                previewStats?.atk || null,
                "#f87171"
              )}
              {renderStatWithPreview(
                "MATK",
                matk,
                previewStats?.matk || null,
                "#c084fc"
              )}
              {renderStatWithPreview(
                "DEF",
              ...
              >
                {character.jobClass === "Swordsman" ||
                character.jobClass === "Knight"
                  ? "STR increases ATK greatly"
                  : ""}
                {character.jobClass === "Archer" ||
                character.jobClass === "Hunter"
                  ? "DEX & AGI increase ATK"
                  : ""}
                {character.jobClass === "Mage" ||
                character.jobClass === "Wizard"
        ...
        >
          ...
          >
            ...
            {[
              ...
              {
                key: "garment" as const,
                label: "Garment",
                icon: "🧥"
              },
              {
                key: "footgear" as const,
                label: "Footgear",
                icon: "👢"
              },
              {
              ...
              const SLOT_RARITY_COLORS: Record<
                string,
                string
              > = {
                legendary: "#ff6b35",
              ...
              const rarityColor = item
                ? SLOT_RARITY_COLORS[item.rarity] ||
                  "#9ca3af"
                : "#334155";
        ...
        >
          <AchievementPanel
            ...
            progress={achievementData.stats}
          />
        </div>
      </div>
